...
const getFingerForChar = (char) => {
  ...
  if (c === ' ') return { name: '親指', hand: 'どちらかの', id: '親指' };
  if ('1!qaz'.includes(c)) return { name: '小指', hand: '左手', id: '左手・小指' };
  if ('2"wsx'.includes(c)) return { name: '薬指', hand: '左手', id: '左手・薬指' };
  if ('3#edc'.includes(c)) return { name: '中指', hand: '左手', id: '左手・中指' };
  if ('4$5%rtfgvb'.includes(c)) return { name: '人差し指', hand: '左手', id: '左手・人差し指' };
  if ('6&7\'yuhjnm'.includes(c)) return { name: '人差し指', hand: '右手', id: '右手・人差し指' };
  if ('8(ik,<'.includes(c)) return { name: '中指', hand: '右手', id: '右手・中指' };
  if ('9)ol.>'.includes(c)) return { name: '薬指', hand: '右手', id: '右手・薬指' };
  if ('0p;/-_=+[]{}\\|:\'"?!~¥@^'.includes(c)) return { name: '小指', hand: '右手', id: '右手・小指' };
